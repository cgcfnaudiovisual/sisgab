...
class DataService:
    ...
    def get_alunos_data(self, force_refresh: bool = False) -> pd.DataFrame:
        ...
        if not df.empty:
            # Filtrar alunos com pelotão ou status como "BAIXA", "NÃO SE APRESENTOU" ou "NAO SE APRESENTOU"
            exclude_values = ['BAIXA', 'NÃO SE APRESENTOU', 'NAO SE APRESENTOU']
            if 'pelotao' in df.columns:
                mask_pelotao = df['pelotao'].astype(str).str.strip().str.upper().isin(exclude_values)
                df = df[~mask_pelotao]
            if 'status' in df.columns:
                mask_status = df['status'].astype(str).str.strip().str.upper().isin(exclude_values)
                df = df[~mask_status]
        return df
