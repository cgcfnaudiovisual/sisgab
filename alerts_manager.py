...
# Cache em memória para evitar leituras/escritas excessivas no disco
_alerts_config_cache = None

def load_alerts_config() -> dict:
    """Carrega as configurações de som e agendamento de alertas a partir do Supabase ou local."""
    global _alerts_config_cache
    if _alerts_config_cache is not None:
        return _alerts_config_cache
    ...
    try:
        ...
        if db_conn:
            ...
            if res.data:
                ...
                # Só escreve no arquivo local se ele não existir ou se o conteúdo for diferente,
                # para evitar disparar recarregamento de watchers de arquivos (como nodemon/pm2)
                try:
                    should_write = True
                    if os.path.exists(ALERTS_CONFIG_PATH):
                        with open(ALERTS_CONFIG_PATH, "r", encoding="utf-8") as f:
                            existing_data = json.load(f)
                        if existing_data == merged:
                            should_write = False
                    if should_write:
...
def save_alerts_config(config: dict):
    ...
    global _alerts_config_cache
    _alerts_config_cache = config.copy()
    try:
        with open(ALERTS_CONFIG_PATH, "w", encoding="utf-8") as f:
...
class AlertsManager:
    ...
    def calculate_naval_bell_strikes(cls, dt: datetime) -> int:
        """Calcula a quantidade de baladas de sino baseada no quarto naval customizado do SisGAB."""
        hour = dt.hour
        minute = dt.minute
    ...
    def trigger_alert(cls, title: str, message: str, type_: str = 'info', extra_options: dict = None):
        ...
        # Resolve se for uma sequência de som personalizada
        custom_sequences = config.get("custom_sequences", {})
        if isinstance(mapped_sound, str) and mapped_sound in custom_sequences:
            mapped_sound = custom_sequences[mapped_sound]
