            ...
            with ui.tab_panels(tabs_p, value='proc').classes('w-full bg-transparent p-0'):
                ...
                with ui.tab_panel('cfg').classes('w-full p-2 gap-4'):
                    ...
                    async def salvar_configuracoes():
                        ...
                        ok = await asyncio.to_thread(update_public_event, slug, novos_dados)
                        if ok:
                            ui.notify('✅ Configurações e PIN do Portal salvos com sucesso!', color='positive')
                        else:
                            ui.notify('❌ Erro ao salvar configurações.', color='negative')
