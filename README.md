...
# Guia de Implantação SisGAB (Hugging Face Spaces)

Este documento descreve os passos para implantar o aplicativo **SisGAB (NiceGUI)** nos **Hugging Face Spaces** utilizando **Docker**, aproveitando a infraestrutura gratuita de **16GB de RAM**.
...
3. Defina as configurações iniciais:
   * **Space Name**: Nome de sua preferência (ex: `sisgab-app`).
   ...
   * **Visibility**: Selecione **"Private"** (Garante total segurança de acesso aos dados militares do Gabinete).
...
1. **Construa a imagem**:
   ```bash
   docker build -t sisgab-hf .
   ```

2. **Rode o container na porta 7860**:
   ```bash
   docker run -d -p 7860:7860 --name sisgab_local --env-file .env -e PORT=7860 -e HOST=0.0.0.0 sisgab-hf
   ```
